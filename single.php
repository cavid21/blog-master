<?php
    ...
    $query1 = $db->select("SELECT * FROM `news` WHERE id=$id");
    $query2 = $db->select("SELECT `title` FROM `news` ORDER BY id DESC LIMIT 5");
    $query3 = $db->select("SELECT `count_view` FROM `news` WHERE id=$id");
    $query4 = $db->select("SELECT `title` FROM `news` ORDER BY count_view DESC LIMIT 5");
    $row = mysqli_fetch_array($query3);
    $count_view = $row['count_view'] + 1;
    $db->query("UPDATE `news` SET `count_view`='$count_view' WHERE id=$id");
        ...
        <div class="content">
            <?php $row = mysqli_fetch_array($query1); ?>
                <div class="box1">
                    ...
                    <span>
                        <?php
                            echo "By ".$row['admin_name']." at -> ".$row['news_adding_time']."";
                        ?>
                    </span>
                    <div class="box1_img">
